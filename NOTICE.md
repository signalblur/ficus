...
by `scripts/build-demo.sh` (published as `docs/dashboard.html`), so it inherits
the credits above. The landing page at `docs/index.html` is generated by
`scripts/build-docs.sh` from the same data files and carries no artwork.
