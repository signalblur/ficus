...
while IFS= read -r JSONL_FILE; do
  ...
  # Insert into DB
  sqlite3 "$DB_PATH" "INSERT OR IGNORE INTO sessions (session_id, project, model, input_tokens, output_tokens, cache_read_tokens, cache_creation_tokens, cost_usd, co2_grams, started_at, ended_at, source, methodology_version, excluded, energy_wh, water_ml, embodied_gco2e) VALUES ('${SESSION_ID}', '${PROJECT}', '${MODEL_RAW}', ${TOTAL_INPUT}, ${OUTPUT_TOKENS}, ${CACHE_READ}, ${CACHE_CREATION}, ${COST_USD}, ${CO2_G}, '${FIRST_TS}', '${LAST_TS}', 'backfill', ${METHODOLOGY_VERSION}, ${EXCLUDED}, ${ENERGY_WH}, ${WATER_ML}, ${EMBODIED_G});" 2>/dev/null || { ERRORS=$((ERRORS + 1)); continue; }
